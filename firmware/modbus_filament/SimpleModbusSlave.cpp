...
// The maximum Serial1 ring buffer size is 128
...
unsigned int modbus_update(unsigned int *holdingRegs)
{
  ...
  {
    // The maximum number of bytes is limited to the Serial1 buffer size of 128 bytes
    // If more bytes is received than the BUFFER_SIZE the overflow flag will be set and the 
    // Serial1 buffer will be red untill all the data is cleared from the receive buffer.
    if (overflow) 
      Serial1.read();
    else
    {
